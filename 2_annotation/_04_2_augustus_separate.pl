...
die("Usage: perl $0 <Dir prefix(eg. Dir prefix of EC.1 is EC) <Fasta prefix(SeparateFile)> <Separate number> <Ref Species> <Out file> <PBS template>") if @ARGV != 6;

my($dir_prefix, $fa_prefix, $num, $ref, $outfile, $pbs) = @ARGV;

for(my $i = 1; $i <= $num; $i++){
        print "Solving dir $dir_prefix.$i\n";
        `qsub -N "augustus_$i" -o "log_augustus_$i" -v species=$ref,fastfile="$dir_prefix.$i/$fa_prefix.$i",output="$dir_prefix.$i/$outfile" $pbs`;
}
